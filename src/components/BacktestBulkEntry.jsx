...
let nextId = 1;
function emptyRow() {
  return { id: nextId++, direction: 'LONG', entryPrice: '', atr: '', slMultiple: '1.0', tpMultiple: '2.0', leverage: '1', outcome: 'WIN' };
}
...
export default function BacktestBulkEntry({ startingCapital, onRunBacktest }) {
  ...
  function runBacktest() {
    const validRows = rows.filter(r => r.entryPrice && r.atr && parseFloat(r.entryPrice) > 0 && parseFloat(r.atr) > 0);
  ...
  const validCount = rows.filter(r => r.entryPrice && r.atr && parseFloat(r.entryPrice) > 0).length;
  ...
  return (
    <div className="terminal-panel">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '860px', fontFamily: "'Roboto Mono', monospace", fontSize: '0.75rem' }}>
          ...
          <tbody>
            {rows.map((row, idx) => (
              ...
              >
                <td style={{ padding: '0.375rem 0.625rem', color: '#444444' }}>{idx + 1}</td>
                ...
                <td style={{ padding: '0.375rem 0.375rem' }}>
                  <select value={row.tpMultiple} onChange={e => updateRow(row.id, 'tpMultiple', e.target.value)} style={cellSelect}>
                    {TP_OPTIONS.map(o => <option key={o} value={o}>{o}x</option>)}
                  </select>
                </td>
                <td style={{ padding: '0.375rem 0.375rem' }}>
      ...
      <div style={{ padding: '1rem', borderTop: '1px solid #2a2a2a' }}>
        <button
          className="btn-amber"
          style={{ width: '100%', padding: '0.875rem', fontSize: '0.9375rem', opacity: validCount === 0 ? 0.3 : 1, cursor: validCount === 0 ? 'not-allowed' : 'pointer' }}
          onClick={runBacktest}
...
function ActionBtn({ onClick, danger, children }) {
  return (
    <button
      ...
      style={{
        ...actionBtnStyle,
        borderColor: danger ? '#FF1744' : '#FF6600',
        color: danger ? '#FF1744' : '#FF6600',
      }}
      onMouseEnter={e => {
        e.currentTarget.style.background = danger ? '#FF1744' : '#FF6600';
        e.currentTarget.style.color = danger ? '#ffffff' : '#000000';
      }}
      onMouseLeave={e => {
        e.currentTarget.style.background = 'transparent';
        e.currentTarget.style.color = danger ? '#FF1744' : '#FF6600';
      }}
    >
      {children}
    </button>
