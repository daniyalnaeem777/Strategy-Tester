# ================================================================
# TP/SL Calculator (Live + Backtest) — Final Stable Version
# Designed for Daniyal Naeem | Streamlit app
# ================================================================
# ✅ Features:
# - Live Mode: quick TP/SL calculator
# - Backtest Mode: record trades, compound equity, export PDF report
# - Shows table, summary & equity curve ONLY after End Session
# - Helvetica theme, clean rectangular sections
# - No pie chart (replaced with total stats summary)
# ================================================================
        ...
        with st.container(border=True):
            ...
            if st.button("Extract Report (PDF)", use_container_width=True):
                if not REPORTLAB_OK or plt is None:
                    st.error("Install reportlab & matplotlib to export PDF.")
                else:
                    buf_eq = BytesIO()
                    fig.savefig(buf_eq, format="png", bbox_inches="tight", dpi=200)
                    buf_eq.seek(0)
                    ...
                    story.append(Paragraph(f"Total Trades: <b>{total}</b><br/>"
                                           f"Winning Trades: <b>{wins}</b><br/>"
                                           f"Winning Percentage: <b>{win_pct:.2f}%</b>", body))
