# tp_sl_calculator_clean.py — Single-border sections, Helvetica, no stray boxes
...
# ---------- CSS: only our .section gets a border ----------
st.markdown("""
<style>
  * { font-family: Helvetica, Arial, sans-serif !important; }
  ...
  /* ONE border per section */
  .section { border:1px solid rgba(255,255,255,0.85); border-radius:14px; padding:12px 14px; margin:12px 0; }

  /* kill any accidental borders from earlier attempts (but keep input borders) */
  div[data-testid="stVerticalBlock"] > div:has(> div[style*="border: 1px solid"]) { border: none !important; }
  div[style*="border: 1px solid rgba(255, 255, 255, 0.85)"]:not(.section) { border:none !important; }
...
def _pct_from_exit(entry, exit_price, side):
    return ((exit_price - entry)/entry)*100 if side=="Long" else ((entry - exit_price)/entry)*100
...
if mode == "Live":
    ...
    st.markdown(f"<span class='chip'>Current SL = {sl_mult_live} × ATR</span><span class='chip'>TP = {TP_MULT} × ATR</span>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    # Entry + ATR
    st.markdown("<div class='section'>", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
    with c1: entry_live = st.number_input("Entry Price", min_value=0.0, format=f"%.{DEC}f")
    with c2: atr_live   = st.number_input("ATR (14)",     min_value=0.0, format=f"%.{DEC}f")
    st.markdown("</div>", unsafe_allow_html=True)
...
if mode == "Backtest":
    ...
    if start_clicked:
        if start_equity<=0: st.error("Please enter a positive account size.")
        else:
    ...
    st.markdown(f"<span class='chip'>Current SL = {sl_mult} × ATR</span><span class='chip'>TP = {TP_MULT} × ATR</span>", unsafe_allow_html=True)

    c1,c2 = st.columns(2)
    with c1: entry = st.number_input("Entry Price", min_value=0.0, format=f"%.{DEC}f")
    with c2: atr   = st.number_input("ATR (14)",     min_value=0.0, format=f"%.{DEC}f")
    ...
    if entry>0 and atr>0:
        ...
        with cA:
            if st.button("Record Win ✅", use_container_width=True, disabled=not st.session_state.bt["recording"]):
                ...
                _log_trade("WIN", calc["side"], calc["entry"], calc["atr"], calc["sl_mult"], calc["sl"], calc["tp"], calc["rr"], None, calc["tp_pct"])
        ...
        with cB:
            if st.button("Record Loss ❌", use_container_width=True, disabled=not st.session_state.bt["recording"]):
                ...
                _log_trade("LOSS", calc["side"], calc["entry"], calc["atr"], calc["sl_mult"], calc["sl"], calc["tp"], calc["rr"], None, -calc["sl_pct"])
        ...
        with cC:
            if st.button("Closed at selected price 🟩", use_container_width=True, disabled=not st.session_state.bt["recording"]):
                if exit_price<=0:
                    ...
                    _compound(pct/100.0)
                    _log_trade("CLOSED", calc["side"], calc["entry"], calc["atr"], calc["sl_mult"], calc["sl"], calc["tp"], calc["rr"], exit_price, pct)
    ...
    if st.session_state.bt["summary_ready"]:
        ...
        if plt is None:
            st.error("Matplotlib required for styled pie. Add `matplotlib` to requirements.txt.")
        else:
            fig, ax = plt.subplots()
            fig.patch.set_facecolor("black"); ax.set_facecolor("black")
            wedges, _, _ = ax.pie([wins, losses], colors=["#00c853","#ff1744"],
        ...
        if plt is None:
            ...
            ax2.set_xlabel("Trades"); ax2.set_ylabel("Account Value")
