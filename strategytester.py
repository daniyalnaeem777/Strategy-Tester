# ladder_calculator.py — Ladder Calculator (indicators in one straight line)
...
<style>
  * { font-family: Helvetica, Arial, sans-serif !important; }
  ...
  /* Indicator label system — exact same height + no wrap so all inputs align */
  .field-wrap { display:flex; flex-direction:column; align-items:flex-start; }
  .lbl { 
    font-weight: 700; 
    font-size: 0.90rem;          /* slightly smaller to keep one line */
    line-height: 1;              
    height: 18px;                /* <— uniform label height */
    white-space: nowrap;         /* <— no line breaks */
    margin: 2px 0 8px 0;         /* small gap above inputs */
  }
...
with st.container(border=True):
    ...
    with c1:
        st.markdown("<div class='lbl'>Market Price</div>", unsafe_allow_html=True)
        market = st.number_input("Market Price", min_value=0.0, format="%.4f", key="mkt", label_visibility="collapsed")
    with c2:
        st.markdown("<div class='lbl'>Upper Zone (UZ)</div>", unsafe_allow_html=True)
        zone_upper = st.number_input("Upper Zone", min_value=0.0, format="%.4f", key="zu", label_visibility="collapsed")
    with c3:
        st.markdown("<div class='lbl'>Lower Zone (LZ)</div>", unsafe_allow_html=True)
        zone_lower = st.number_input("Lower Zone", min_value=0.0, format="%.4f", key="zl", label_visibility="collapsed")

# ============= 3️⃣ Technical Indicators (one straight horizontal line) =============
with st.container(border=True):
    st.markdown("### **Technical Indicators**")

    # Equal column widths to keep perfect alignment
    atr_col, macd_col, adx_col, rsi_col = st.columns([1,1,1,1])

    # Left (Compulsory)
    with atr_col:
        st.markdown("<div class='lbl'>ATR (4h, 14)</div>", unsafe_allow_html=True)
        atr = st.number_input("ATR", min_value=0.0, format="%.4f", key="atr", label_visibility="collapsed")

    with macd_col:
        st.markdown("<div class='lbl'>MACD (1h, 12-26-9)</div>", unsafe_allow_html=True)
        macd = st.selectbox("MACD", ["Neutral", "Bullish", "Bearish"], label_visibility="collapsed")

    # Right (Optional) — keep single line using abbreviated “(Opt.)”
    with adx_col:
        st.markdown("<div class='lbl'>ADX (4h, 14) (Opt.)</div>", unsafe_allow_html=True)
        adx = st.number_input("ADX", min_value=0.0, value=0.0, step=0.5, format="%.2f", key="adx", label_visibility="collapsed")

    with rsi_col:
        st.markdown("<div class='lbl'>RSI-3 Trigger (Opt.)</div>", unsafe_allow_html=True)
...
def ladder_count(zone_w: float, atr_val: float, adx_val: float):
    ...
    if adx_val >= 25: base = max(2, base - 1)
    return base, k
